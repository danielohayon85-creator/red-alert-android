...
fun SettingsScreen(modifier: Modifier = Modifier) {
    ...
    when (destination) {
        SettingsDestination.MENU -> Column(modifier = modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            Text(
...
private fun WithBackButton(onBack: () -> Unit, content: @Composable () -> Unit) {
    Column(modifier = Modifier.fillMaxSize()) {
        TextButton(onClick = onBack) { Text(text = stringResource(R.string.settings_back)) }
        content()
    }
}
...
private fun SettingsMenuItem(label: String, onClick: () -> Unit) {
    Card(
        ...
        Text(text = label, style = MaterialTheme.typography.titleLarge, modifier = Modifier.padding(16.dp))
    }
}
